function [stim] = nVoke_visual_stim_info_Sz(VS_info,Opt)
...
stim.Opt = Opt; 
stim.feature = VS_info.feature; %'FillOval' or 'FIllRect
stim.rep = VS_info.rep;
stim.time.ON = VS_info.on; %stimulus-duration (sec). Caution FRAME RATE
stim.Sz_center = VS_info.center;
stim.Sz_centroid = [grid.position.w_column(VS_info.center)'; grid.position.h_column(VS_info.center)]; %row1:x, row2:y
...
stim.color = colval(fix(prctile(colval,VS_info.stim_contrast))+1)';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
