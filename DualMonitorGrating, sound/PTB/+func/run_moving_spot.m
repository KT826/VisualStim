function [OUTPUT] = run_moving_spot(stim, dio, monitor, grid, expWin)
...
try 
    for rep = 1 : stim.rep
        for swp = 1 : size(stim.trial_order_pix{rep},1) 
            ...
            outputSingleScan(dio.TTL.trigger,[1]) % TTL output
            outputSingleScan(dio.TTL.w,[1]) % TTL output
            outputSingleScan(dio.TTL.trigger,[0]) % TTL output
            ...
            while move_end > (move_pix * n_disp_frame) %刺激がmonitor幅より短い時にON
                ...
                Screen(stim.feature, expWin, spot_color, centeredRect);
