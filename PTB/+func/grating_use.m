...
%%
swp = 6; rep = 1;
direction = stim.trial_order{rep,1}.direction(swp);
cyclespersecond = stim.trial_order{rep,1}.TempFreq(swp); %Hz(temporal frequency)
f = stim.trial_order{rep,1}.SpaFreq_cpp(swp); %c/pixel (spatial frequency)
duration = stim.trial_order{rep,1}.Duration(swp); %duration of stimulus
contrast = stim.trial_order{rep,1}.StimColor_Contrast(swp);%
...
contrast = 127*(1-contrast/100);
white=WhiteIndex(screenNumber)-contrast;
black=BlackIndex(screenNumber)+contrast;
...
while vbl < vblendtime
    ...
    if drawmask==1
        ...
        Screen('DrawTexture', w, masktex, [0 0 visiblesize visiblesize], dstRect, direction);
    end
